...
console.log('=== Checking Cell Values vs Cell Comments for D/L or Delay ===');
...
wb.SheetNames.forEach(sheetName => {
  ...
  data.forEach((row, rowIdx) => {
    ...
    shifts.forEach(s => {
      ...
      if (delayMin > 0) {
        // Check cell values AND comments in row
        const cellVals = [];
        const cellComments = [];

        s.commentCols.forEach(colIdx => {
          const val = String(row[colIdx] || '').trim();
          if (val) cellVals.push(val);
          ...
          if (cell && cell.c && cell.c.length > 0) {
            ...
            if (cleaned) cellComments.push(cleaned);
          }
        });

        console.log(`Sheet '${sheetName}' [${mc}][${s.name}] DelayMin=${delayMin}:`);
        console.log(`   Cell values:`, cellVals);
