...
export default function DataExporter() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Date Range Selector & Presets Control Panel */}
      <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          {/* Quick Date Presets */}
          <div>
            <label className="text-xs font-extrabold text-slate-500 uppercase tracking-wider block mb-2">
              ⚡ Quick Select Range (เลือกช่วงวันที่ด่วน):
            </label>
            <div className="flex flex-wrap items-center gap-2">
              <button
                onClick={() => handlePresetSelect('1-3')}
                className="px-3.5 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded-xl text-xs font-extrabold transition-all"
              >
                📅 วันที่ 1 - 3
              </button>
              <button
              ...
              <button
                ...
                className="px-3.5 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-xl text-xs font-extrabold transition-all"
              >
                🗓️ ย้อนหลัง 7 วัน
              </button>
              <button
                onClick={() => handlePresetSelect('month')}
                className="px-3.5 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-xl text-xs font-extrabold transition-all"
              >
                📆 ทั้งเดือนนี้ (1-30)
              </button>
              <button
                onClick={() => handlePresetSelect('today')}
                className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 rounded-xl text-xs font-extrabold transition-all"
              >
                🎯 วันนี้วันเดียว
              </button>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div>
              ...
              <input
                ...
                className="bg-slate-50 border border-slate-300 text-slate-800 rounded-xl px-3 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-blue-500/30"
              />
            </div>
            <span className="text-slate-400 font-bold self-end mb-2">ถึง</span>
            <div>
            ...
            <button
              ...
              className="self-end p-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition-all shadow-sm mb-0.5"
              title="Refresh Data"
            >
