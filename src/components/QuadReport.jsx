    ...
    <div className="bg-white rounded-xl shadow-xs border border-slate-100 p-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold text-slate-800 text-sm md:text-base">QUAD Performance & Output</h3>
            <p className="text-[11px] text-slate-400">ข้อมูล OEE และยอดผลิตแยกกะจาก Quad Booker Sheet</p>
          </div>
        </div>
        ...
        {hasOutput && (
          <div className="flex items-center gap-1.5 bg-indigo-50 border border-indigo-100 rounded-lg px-2.5 py-1">
            ...
            <span className="text-sm font-black text-indigo-700">{output.grandTotal.toLocaleString()}</span>
      ...
      {hasOee ? (
        <div className="grid grid-cols-4 sm:grid-cols-7 gap-1.5">
          <div className="bg-gradient-to-br from-indigo-50 to-indigo-100/50 border border-indigo-200/60 rounded-lg p-2 text-center min-w-0">
            <p className="text-[9px] font-bold text-indigo-600 uppercase tracking-tight whitespace-nowrap">OEE2</p>
            <p className="text-sm sm:text-base font-black text-indigo-900 whitespace-nowrap">{oee.oee2_pct}%</p>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {output.codeBreakdown.map((item, idx) => (
              ...
              >
                ...
                <span className="text-[10px] font-semibold text-indigo-600 bg-indigo-50 px-1 rounded">
                  {item.percentage}%
                </span>
              </div>
            ))}
            ...
            return (
              <div key={shiftNum} className="border border-slate-200/70 rounded-lg overflow-hidden bg-slate-50/40 flex flex-col">
                <div className="bg-slate-100/80 px-2.5 py-1.5 border-b border-slate-200/70 flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-black text-indigo-700 bg-white px-1.5 py-0.5 rounded border border-slate-200">
