...
export default function SafetyLspReport() {
  ...
  const defaultWorkers = [
    ...
    { id: 24, legacy: '10062', name: 'Phichet Dee-On (พิเชษฐ์ ดีอ่อน)', dept: 'BCA-Q', areaCode: '1022', isLspTarget: true, monthly: { JAN: 4, FEB: 4, MAR: 4, APR: 5, MAY: 4, JUN: 4, JUL: 4, AUG: 4, SEP: 0, OCT: '', NOV: '', DEC: '' }, ytdPct: 71 },
    { id: 25, legacy: '12445', name: 'Pinthusorn Prasertpolkrang (พินทุสร ประเสริฐพลกรัง)', dept: 'BCA-Q', areaCode: '1022', isLspTarget: true, monthly: { JAN: 4, FEB: 4, MAR: 5, APR: 4, MAY: 4, JUN: 4, JUL: 4, AUG: 4, SEP: 2, OCT: '', NOV: '', DEC: '' }, ytdPct: 73 },
    { id: 35, legacy: '10558', name: 'Suruttapong Siriluk (สุรุจพงศ์ ศิริลักษณ์)', dept: 'BCA-E', areaCode: '1100', isLspTarget: true, monthly: { JAN: 3, FEB: 4, MAR: 4, APR: 4, MAY: 0, JUN: 5, JUL: 3, AUG: 6, SEP: 1, OCT: '', NOV: '', DEC: '' }, ytdPct: 63 },
    { id: 41, legacy: '1308', name: 'Vinai Fuangfoo (วินัย ฟักฟู)', dept: 'BCA-HR', areaCode: '1053', isLspTarget: true, monthly: { JAN: 5, FEB: 4, MAR: 4, APR: 5, MAY: 4, JUN: 4, JUL: 4, AUG: 4, SEP: 0, OCT: '', NOV: '', DEC: '' }, ytdPct: 75 },
    { id: 43, legacy: '12001', name: 'Wata Phattanapong (วรา พัฒนพงศ์)', dept: 'BCA-EHS', areaCode: '1056', isLspTarget: true, monthly: { JAN: 4, FEB: 5, MAR: 4, APR: 4, MAY: 4, JUN: 6, JUL: 4, AUG: 5, SEP: 2, OCT: '', NOV: '', DEC: '' }, ytdPct: 75 }
  ];
  ...
  const departments = data ? ['ALL', ...new Set(data.workers.filter(w => String(w.id) !== '20' && w.legacy !== '12925' && !w.dept.toUpperCase().includes('BCB')).map(w => w.dept).filter(Boolean))] : ['ALL'];
  ...
  const filteredWorkers = data ? data.workers.filter(w => {
    ...
    if (w.dept && w.dept.toUpperCase().includes('BCB')) return false;
